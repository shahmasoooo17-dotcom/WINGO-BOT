...
*Admin will review your feedback soon.*`,
        ...
        // Fixed undefined reference crash here by reading directly from storage object
        ADMIN_IDS.forEach(adminId => {
            bot.sendMessage(adminId,
`💬 *NEW FEEDBACK RECEIVED!*
━━━━━━━━━━━━━━━━━━━━
📝 *ID:* #${feedbackId}
👤 *User:* ${feedbacks[feedbackId].name} (@${feedbacks[feedbackId].username || 'N/A'})
🆔 *ID:* \`${userId}\`
⭐ *Rating:* ${rating} Stars
💬 *Message:* ${feedbacks[feedbackId].message}
━━━━━━━━━━━━━━━━━━━━
...
function handleMessage(msg) {
    ...
    // Fixed flow: If user clicks send without custom period, fallback safely to the active calculated period
    if (state === 'wingo_30s_predict') {
        let periodStr = text;
        if (!/^\d{8,14}$/.test(periodStr)) {
            periodStr = getCurrentPeriod30s();
        }
        handleWingoPrediction(userId, periodStr, true);
        userStates[userId] = null;
        return;
    }
    if (state === 'wingo_1m_predict') {
        let periodStr = text;
        if (!/^\d{8,14}$/.test(periodStr)) {
            periodStr = getCurrentPeriod1m();
        }
        handleWingoPrediction(userId, periodStr, false);
        userStates[userId] = null;
        return;
    }
...
bot.on('message', (msg) => {
    ...
    // Fixed duplicate messaging response error by handling text-exclusive contents
    if (msg.text) {
        handleMessage(msg);
    }
});
