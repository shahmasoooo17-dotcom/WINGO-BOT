// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// ENTERPRISE UNIFIED TRADING BOT v3.4 - PAKISTAN WINGO STANDARD (BUTTON EDITION)
// - Full HTML Core Integration (Zero layout parsing crashes)
// - Inline UI Keyboards (Interactive button paths)
// - Real-time PKT (UTC+5) Timezone Synchronization Engine
// - 14-Digit Standardized Indian/Pakistani WinGo Period Parser
// - One-Tap Continuous Sequential Prediction Loop Integration
// - Fully Automated Button-Driven Verification (No Manual Text Writing Required)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ── Live Calculation Engines (Pakistan Time Synchronization) ───────────────────
...
function getCurrentPeriod30s() {
    ...
    const dateStr = `${pkt.getFullYear()}${pad(pkt.getMonth() + 1)}${pad(pkt.getDate())}`;
    
    const totalSeconds = (pkt.getHours() * 3600) + (pkt.getMinutes() * 60) + pkt.getSeconds();
    const slot = Math.floor(totalSeconds / 30) + 1;
    
    return `${dateStr}30${String(slot).padStart(4, '0')}`;
}
...
function getCurrentPeriod1m() {
    ...
    const slot = (pkt.getHours() * 60) + pkt.getMinutes() + 1;
    
    return `${dateStr}01${String(slot).padStart(4, '0')}`;
}
...
// Deterministic Pseudo-Random Kernel
...
👉 ${outcome.detail}`, { 
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: `⏩ Next Prediction (${nextSequence.slice(-4)})`, callback_data: `wpred:${modeKey}:${nextSequence}` }],
...
function today() { return getPKTTime().toISOString().slice(0, 10); }
...
bot.on('message', (msg) => {
    ...
    if (!allUsers[userId]) allUsers[userId] = { name, username: uname, joinDate: today(), wingoPredictions: 0 };
...
──────────────────────────────
Premium grade tracking predictive algorithms. Engineered for seamless automated signals synchronized to Pakistan Standard Time.
...
──────────────────────────────
🎰 Wingo Logs: <b>${allUsers[userId].wingoPredictions || 0} hits</b>`, { parse_mode: 'HTML' });
            break;
        case '❓ SUPPORT CENTRE':
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('wmenu:')) {
        ...
        const livePeriod = is30s ? getCurrentPeriod30s() : getCurrentPeriod1m();
        const timeRemaining = is30s ? getTimeLeft30s() : getTimeLeft1m();

        bot.sendMessage(userId,
`<b>🎯 MODE SELECTED: ${is30s ? '30 SEC' : '1 MIN'} WINGO</b>
──────────────────────────────
⏱ PKT Countdown : <b>~${timeRemaining}s</b>
📌 Projected 14-Digit Period: <code>${livePeriod}</code>
──────────────────────────────
...
2️⃣ Tap the validation verification button below.
3️⃣ Deliver image proof screenshot to terminal workspace.

Your Token ID: <code>${userId}</code>`, { 
                parse_mode: 'HTML',
                reply_markup: {
                    inline_keyboard: [
                        [{ text: '✅ I Have Paid (Verify)', callback_data: 'click_paid_button' }]
                    ]
                }
            });
        }
    ...
    // [User Path] Automated Payment Action Trigger Button
    if (data === 'click_paid_button') {
        triggerLegacyPaidAction(userId);
    }
...
⏳ <i>Algorithmic tracking checker active. Results will print automatically upon game conclusion.</i>`, { 
        ...
        reply_markup: {
            inline_keyboard: [
                [{ text: `⏩ Next Prediction (${nextSequence.slice(-4)})`, callback_data: `wpred:${mode}:${nextSequence}` }],
...
// Handle administrative processing sequences
