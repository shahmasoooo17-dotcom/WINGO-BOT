...
// ======================= IN-MEMORY STORAGE =======================
const premiumUsers    = {};
const freeUsage       = {};     // free predictions counter for wingo
const userStates      = {};
...
const adminSessions   = {};
const feedbacks       = {};
const predictions     = {};     // for 1-min auto results
...
// ======================= AUTO RESULT FETCHER (CORS PROXY) =======================
...
function startAutoResultChecker() {
    ...
    autoResultInterval = setInterval(fetchWingo1MResults, 15000);
...
function showApprovePaymentMenu(adminId) {
    ...
    if (pendings.length === 0) return bot.sendMessage(adminId, '✅ No pending payments.', adminMenu());
...
function doApprovePayment(adminId, targetIdStr) {
    ...
    if (isNaN(targetId)) return bot.sendMessage(adminId, '❌ Invalid ID.', adminMenu());
    const pending = pendingPayments[targetId];
    if (!pending) return bot.sendMessage(adminId, `❌ No pending payment for ${targetId}`, adminMenu());
    ...
    bot.sendMessage(adminId, `✅ Premium activated for ${targetId}\nExpires: ${expiry}`, adminMenu());
    bot.sendMessage(targetId, `🎉 PREMIUM ACTIVATED! Expires: ${expiry}\n\nUnlimited Wingo predictions!`);
    userStates[adminId] = 'admin_panel';
}
function showRemovePremiumMenu(adminId) {
    if (Object.keys(premiumUsers).length === 0) return bot.sendMessage(adminId, '📭 No premium users.', adminMenu());
    userStates[adminId] = 'waiting_remove_id';
    bot.sendMessage(adminId, '❌ Enter user ID to remove premium:', { reply_markup: { force_reply: true } });
}
function doRemovePremium(adminId, targetIdStr) {
    const targetId = parseInt(targetIdStr);
    if (isNaN(targetId) || !premiumUsers[targetId]) return bot.sendMessage(adminId, '❌ Invalid or non-premium user.', adminMenu());
    delete premiumUsers[targetId];
    bot.sendMessage(adminId, `✅ Premium removed for ${targetId}`, adminMenu());
...
function showBroadcastMenu(adminId) {
    ...
    bot.sendMessage(adminId, '📢 Enter broadcast message:', { reply_markup: { force_reply: true } });
}
...
${BOT_NAME}
━━━━━━━━━━━━━━━━━━━━
${isPrem ? '💎 PREMIUM MEMBER' : `🆓 FREE – ${WINGO_FREE_LIMIT} predictions/day`}
━━━━━━━━━━━━━━━━━━━━
🎲 WINGO: 30s (manual) / 1m (auto result)
💬 Feedback button
━━━━━━━━━━━━━━━━━━━━`,
...
function cmdWingoMenu(msg) {
    ...
    bot.sendMessage(userId, '🎲 Select prediction type:', wingoMenu());
}
function cmdWingo30(msg) {
    ...
    if (!access.ok) return bot.sendMessage(userId, `⛔ Daily limit reached. /buypremium`, mainMenu(userId));
    userStates[userId] = 'wingo_30s_predict';
    userLastAction[userId] = { type: '30s' };
    bot.sendMessage(userId, `🎯 30 Sec WinGo\n⏱️ Time left: ~${getTimeLeft30s()}s\n📌 Est. period: \`${getCurrentPeriod30s()}\`\n👇 Enter period number:`, { parse_mode: 'Markdown', reply_markup: { force_reply: true } });
}
...
function handleWingoPrediction(userId, periodStr, is30s) {
    ...
    if (!is30s) savePrediction(userId, periodStr, pred.num, pred.color, pred.size, false, isPrem);
    const autoNote = !is30s ? '\n━━━━━━━━━━━━━━━━━━━━\n_✅ You will be notified automatically when result is declared._' : '';
    bot.sendMessage(userId,
`${BOT_NAME} - WINGO
━━━━━━━━━━━━━━━━━━━━
...
function cmdBuyPremium(msg) {
    ...
    bot.sendMessage(userId, '💎 Choose plan:', { reply_markup: keyboard });
}
...
`❓ HELP
🎲 WINGO: 30s (manual period) / 1m (auto result)
💬 FEEDBACK: Send your opinion
👑 Admin: @GojoVipAdmin`,
        { parse_mode: 'Markdown', ...mainMenu(msg.from.id) });
}
function cmdFeedback(msg) {
    userStates[msg.from.id] = 'waiting_feedback_message';
    bot.sendMessage(msg.from.id, '💬 Type your feedback message:', { reply_markup: { force_reply: true } });
}
function processFeedback(userId, name, username, message) {
    ...
    bot.sendMessage(userId, `✅ Feedback saved (ID #${id}). Thank you!`, mainMenu(userId));
...
function cmdAdminLogin(msg) {
    ...
    bot.sendMessage(userId, '🔐 Enter admin password:', { reply_markup: { force_reply: true } });
}
...
function handleMessage(msg) {
    ...
    if (state === 'waiting_feedback_message') { processFeedback(userId, name, uname, text); return; }
    ...
    if (text === '🔙 MAIN MENU') { userStates[userId] = null; bot.sendMessage(userId, 'Main Menu:', mainMenu(userId)); return; }
    if (state === 'wingo_30s_predict' && /^\d{12,14}$/.test(text)) { handleWingoPrediction(userId, text, true); userStates[userId] = null; return; }
    if (state === 'wingo_1m_predict' && /^\d{12,14}$/.test(text)) { handleWingoPrediction(userId, text, false); userStates[userId] = null; return; }
...
bot.on('callback_query', (cq) => {
    ...
    if (data && data.startsWith('plan_')) {
        ...
        if (plan) {
            ...
            bot.sendMessage(userId, `💎 ${plan.name} – PKR ${plan.price}\n📲 EasyPaisa: ${EASYPAISA_NUMBER}\n📲 JazzCash: ${JAZZCASH_NUMBER}\n👤 Name: ${ACCOUNT_NAME}\nAfter payment, type /paid and send screenshot.`);
        }
...
bot.onText(/\/paid/, (msg) => {
    ...
    if (!planKey || !PLANS[planKey]) return bot.sendMessage(userId, '❌ Select a plan first with /buypremium');
    pendingPayments[userId] = { name, date: today(), plan: planKey, screenshot: false };
    userStates[userId] = null;
    bot.sendMessage(userId, '✅ Payment notification sent. Send screenshot.');
    ADMIN_IDS.forEach(adminId => bot.sendMessage(adminId, `💰 Payment claim from ${name} (${userId}) – ${PLANS[planKey].name}`));
});
bot.on('photo', (msg) => {
    ...
    if (pendingPayments[userId]) {
        ...
        bot.sendMessage(userId, '✅ Screenshot received. Admin will verify.');
        ADMIN_IDS.forEach(adminId => {
            bot.forwardMessage(adminId, msg.chat.id, msg.message_id);
            bot.sendMessage(adminId, `📸 Screenshot from ${userId} (${pendingPayments[userId].plan})`);
        });
    }
});
bot.on('message', (msg) => {
    if (msg.chat.type !== 'private') return bot.sendMessage(msg.chat.id, '❌ Private chat only.');
    handleMessage(msg);
});
...
console.log('🚀 Wingo bot started with auto 1-min results');
