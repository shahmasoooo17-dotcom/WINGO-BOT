...
// ======================= PERSISTENT STORAGE =======================
// Files: premiumUsers.json, freeUsage.json, allUsers.json, pendingPayments.json, feedbacks.json, predictions.json, userStates.json
const DATA_FILES = {
...
function loadData() {
    ...
    } catch(e) { console.log('Error loading data:', e.message); }
}
function saveData() {
    try {
...
setInterval(saveData, 30 * 1000); // save every 30 seconds
...
function doApprovePayment(adminId, targetIdStr) {
    ...
    bot.sendMessage(adminId, 
        `✅ *Premium activated for* \`${targetId}\`\n📦 Plan: ${plan.name}\n📅 Expires: ${expiry}`,
...
function cmdStart(msg) {
    ...
    if (!allUsers[userId]) allUsers[userId] = { name, username: uname, joinDate: today(), predictions: 0, saveData };
...
function handleMessage(msg) {
    ...
    if (text === '🔙 MAIN MENU') { userStates[userId] = null; saveData(); bot.sendMessage(userId, 'Main Menu:', mainMenu(userId)); return; }
    if (state === 'wingo_30s_predict' && /^\d{12,14}$/.test(text)) { handleWingoPrediction(userId, text, true); userStates[userId] = null; saveData(); return; }
    if (state === 'wingo_1m_predict' && /^\d{12,14}$/.test(text)) { handleWingoPrediction(userId, text, false); userStates[userId] = null; saveData(); return; }
...
console.log('🚀 Wingo bot started with auto 1-min results (payment system fixed)');