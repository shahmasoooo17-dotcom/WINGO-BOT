...
// ── WINGO PREDICTION FIELD TOGGLES ───────────────────────────────────────────────
// Admin can turn each field ON or OFF from admin panel
let SHOW_NUMBER = true;   // 🔢 Show Number (0-9)
let SHOW_COLOR  = true;   // 🎨 Show Color (Red/Green/Violet)
let SHOW_SIZE   = true;   // 📏 Show Size (Big/Small)
...
const pendingWingoChecks = {}; // { userId: { period, gameMode, prediction } }

// ── Auto Result Checker Functions ────────────────────────────────────────────────
...
async function fetchWingoPeriodFromAPI(market) {
    ...
    catch (e) { console.error('⚠️ Period API error:', e.message); return null; }
}
...
async function fetchWingoResultFromAPI(market, period) {
    ...
    } catch (e) { console.error('⚠️ Result API error:', e.message); return null; }
}
...
function compareWingoPrediction(userPred, apiResult) {
    ...
    const predColor = (userPred.color     || '').toLowerCase();
    const predSize  = (userPred.size      || '').replace(/[📈📉]/g,'').trim().toUpperCase();

    if (predNum === realNum)
        return { outcome:'JACKPOT', emoji:'🎰', label:'JACKPOT! 💰🔥', detail:`Your number *${predNum}* matched exactly! 🎰💰`, realNum, realColor:apiResult.color, realSize };

    const colorMatch =
        (predColor.includes('green')  && realColor.includes('green'))  ||
        (predColor.includes('red')    && realColor.includes('red'))    ||
        (predColor.includes('violet') && realColor.includes('violet'));
    if (colorMatch)
        return { outcome:'WIN', emoji:'✅', label:'WIN! 🎉', detail:`Color matched! *${userPred.color}* → *${apiResult.color}* 🎉`, realNum, realColor:apiResult.color, realSize };

    if (predSize && predSize === realSize)
        return { outcome:'WIN', emoji:'✅', label:'WIN! 🎉', detail:`Size matched! *${predSize}* → *${realSize}* 🎉`, realNum, realColor:apiResult.color, realSize };

    return { outcome:'LOSS', emoji:'❌', label:'LOSS', detail:`Predicted *${userPred.color} / ${predSize}* → Result *${apiResult.color} / ${realSize}*`, realNum, realColor:apiResult.color, realSize };
}
...
async function runWingoAutoChecker() {
    ...
    for (const [userId, check] of entries) {
        try {
            ...
            if (!apiResult || apiResult.digit === undefined) continue;
            ...
            // Show Next Prediction button after result (same game mode)
            const nextKeyboard = {
                reply_markup: {
                    keyboard: [
                        [check.gameMode === '30s' ? '🔄 NEXT 30 SEC PREDICTION' : '🔄 NEXT 1 MIN PREDICTION'],
...
${outcome.detail}
━━━━━━━━━━━━━━━━━━━━
👇 *Tap below for next prediction:*`,
                { parse_mode: 'Markdown', ...nextKeyboard }
            ).catch(() => {});

            delete pendingWingoChecks[userId];
        } catch (err) { console.error(`⚠️ AutoChecker uid=${userId}:`, err.message); }
    }
}
...
function adminMenu() {
    ...
    const numStatus    = SHOW_NUMBER    ? '✅' : '❌';
    const colorStatus  = SHOW_COLOR     ? '✅' : '❌';
    const sizeStatus   = SHOW_SIZE      ? '✅' : '❌';
    return {
        reply_markup: {
            keyboard: [
                ...
                ['🔢 SHOW NUMBER: ' + numStatus, '🎨 SHOW COLOR: ' + colorStatus],
                ['📏 SHOW SIZE: ' + sizeStatus,  '🔍 SIGNAL SETTINGS'           ],
                ['🚪 EXIT ADMIN'                                     ]
            ],
            resize_keyboard: true
        }
...
function handleAdminCommands(userId, text) {
    ...
    // ── PREDICTION FIELD TOGGLES ──────────────────────────────────────────────────
    if (text.startsWith('🔢 SHOW NUMBER:')) {
        SHOW_NUMBER = !SHOW_NUMBER;
        bot.sendMessage(userId,
`🔢 *NUMBER field is now ${SHOW_NUMBER ? 'VISIBLE ✅' : 'HIDDEN ❌'}*\n\nUsers will ${SHOW_NUMBER ? 'see' : 'NOT see'} the predicted number in signals.`,
            { parse_mode: 'Markdown' });
        bot.sendMessage(userId, 'Admin Panel:', adminMenu());
        return;
    }
    if (text.startsWith('🎨 SHOW COLOR:')) {
        SHOW_COLOR = !SHOW_COLOR;
        bot.sendMessage(userId,
`🎨 *COLOR field is now ${SHOW_COLOR ? 'VISIBLE ✅' : 'HIDDEN ❌'}*\n\nUsers will ${SHOW_COLOR ? 'see' : 'NOT see'} Red/Green/Violet in signals.`,
            { parse_mode: 'Markdown' });
        bot.sendMessage(userId, 'Admin Panel:', adminMenu());
        return;
    }
    if (text.startsWith('📏 SHOW SIZE:')) {
        SHOW_SIZE = !SHOW_SIZE;
        bot.sendMessage(userId,
`📏 *SIZE field is now ${SHOW_SIZE ? 'VISIBLE ✅' : 'HIDDEN ❌'}*\n\nUsers will ${SHOW_SIZE ? 'see' : 'NOT see'} Big/Small in signals.`,
            { parse_mode: 'Markdown' });
        bot.sendMessage(userId, 'Admin Panel:', adminMenu());
        return;
    }

    // Signal Settings overview
    if (text === '🔍 SIGNAL SETTINGS') {
        const on  = '✅ ON ';
        const off = '❌ OFF';
        bot.sendMessage(userId,
`🔍 *SIGNAL FIELD SETTINGS*
━━━━━━━━━━━━━━━━━━━━
Controls what appears in user predictions:

🔢 Number (0-9)    : ${SHOW_NUMBER ? on : off}
🎨 Color (Red/Green): ${SHOW_COLOR  ? on : off}
📏 Size (Big/Small) : ${SHOW_SIZE   ? on : off}
━━━━━━━━━━━━━━━━━━━━
💡 *Examples:*
• All ON  → Full signal (number + color + size)
• Only 🎨 → Color only signal
• Only 📏 → Size only signal
• 🎨 + 📏 → Color + Size (no number)
• Only 🔢 → Number only signal
━━━━━━━━━━━━━━━━━━━━
Toggle each field using buttons below 👇`,
            { parse_mode: 'Markdown', ...adminMenu() });
        return;
    }
...
function handleWingoPrediction(userId, periodStr, is30s) {
    ...
    // Save to pending checks for auto result notification
    ...
    // Build prediction message respecting admin field toggles
    let predLines = '';
    if (SHOW_NUMBER) predLines += `🔢 *Number:* *${pred.num}*\n`;
    if (SHOW_COLOR)  predLines += `🎨 *Color:* *${pred.color}*\n`;
    if (SHOW_SIZE)   predLines += `📏 *Size:* *${pred.size}*\n`;
    if (!SHOW_NUMBER && !SHOW_COLOR && !SHOW_SIZE) {
        predLines = `⚠️ _All prediction fields are hidden by admin._\n`;
    }
...
━━━━━━━━━━━━━━━━━━━━
${predLines}💡 Confidence: ${pred.conf}
━━━━━━━━━━━━━━━━━━━━
📊 Remaining: ${remaining}
━━━━━━━━━━━━━━━━━━━━
🔔 *Result alert coming automatically!*
⏳ _WIN / LOSS / JACKPOT will be sent_`,
...
// ── Single unified message handler (handles text + photos) ─────────────────────

bot.on('message', (msg) => {
    if (msg.chat.type !== 'private') {
        bot.sendMessage(msg.chat.id, '❌ This bot only works in private chat!').catch(() => {});
        return;
    }
    ...
    catch (err) { console.error('❌ handleMessage error:', err.message); }
});

console.log('🚀 UNIFIED TRADING BOT v2.5 (STABLE + SIGNAL TOGGLES) is running!');
...
console.log(`🔢 Show Number: ${SHOW_NUMBER} | 🎨 Show Color: ${SHOW_COLOR} | 📏 Show Size: ${SHOW_SIZE}`);
